...
export default function BusinessOffers() {
  ...
  const categories = useMemo(() => [...new Set(offers.map(o => o.category).filter(Boolean))], [offers]);
  const fuelTypes = useMemo(() => [...new Set(offers.map(o => o.fuel_type).filter(Boolean))], [offers]);
      ...
      <div className="pt-24 sm:pt-28 pb-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        ...
        >
          ...
          <p className="mt-2 text-white/50 max-w-xl">
            Scopri la nostra gamma completa di veicoli in NLT. Tutti i prezzi includono assicurazione, manutenzione e soccorso stradale.
          </p>
        </motion.div>
      </div>
