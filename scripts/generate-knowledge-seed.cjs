/**
 * scripts/generate-knowledge-seed.js
 * Generates supabase/migrations/20260622_seed_knowledge_base.sql
 * with structured documents and chunks for Luca's knowledge base.
 * Uses valid UUID formats to avoid PostgreSQL type errors.
 */
...
// Generate valid UUIDs deterministically based on indexes
function getDocUuid(docIdx) {
  const suffix = docIdx.toString().padStart(12, '0');
  return `d0c00000-0000-4000-8000-${suffix}`;
}

function getChunkUuid(docIdx, chunkIdx) {
  const docPart = docIdx.toString().padStart(6, '0');
  const chunkPart = chunkIdx.toString().padStart(6, '0');
  return `c8c00000-0000-4000-8000-${docPart}${chunkPart}`;
}

const documents = [
  {
    title: "Guida ai Carrier NLT in Italia (Arval, Leasys, Ayvens, UnipolRental)",
    source: "manual",
    content: `I principali carrier (società di noleggio) operanti in Italia per il Noleggio a Lungo Termine (NLT) sono Arval, Leasys, Ayvens (nata dalla fusione di ALD Automotive e LeasePlan) e UnipolRental. Ciascun carrier ha regole specifiche, punti di forza e procedure di delibera finanziaria.
...
UnipolRental è l'unico carrier a capitale interamente italiano, parte del Gruppo Unipol. Il suo punto di forza è l'integrazione assicurativa con la rete UnipolSai e l'utilizzo dei centri di riparazione convenzionati MyGlass e officine UnipolService. Offre canoni molto competitivi e un servizio clienti rapido per la gestione dei sinistri.`
  ...
  {
    title: "Requisiti di Approvazione Finanziaria (Privati, P.IVA, Aziende, Garante)",
    source: "manual",
    content: `Per poter sottoscrivere un contratto di Noleggio a Lungo Termine, il cliente (sia privato che azienda) deve superare un'analisi di affidabilità creditizia (chiamata "delibera" o "studio finanziario") effettuata dal carrier per verificare la capacità di pagamento dei canoni mensili.
...
Qualora il richiedente non disponga dei requisiti minimi (ad esempio un privato con contratto di lavoro recente o part-time, o una ditta individuale neonata), è possibile inserire un Garante (Coobbligato). Il garante deve essere una persona fisica con reddito solido e dimostrabile (tramite busta paga a tempo indeterminato o pensione) che firma il contratto impegnandosi a pagare i canoni in caso di inadempienza del locatario principale. I documenti richiesti al garante sono gli stessi previsti per il cliente privato.`
  ...
  {
    title: "I 19 Servizi Offerti da Nolosubito e Coperture Assicurative",
    source: "manual",
    content: `Nolosubito include un pacchetto completo di servizi all'interno del canone mensile di noleggio a lungo termine per garantire una formula "tutto incluso" senza pensieri per l'automobilista. I servizi principali gestiti e offerti sono 19, suddivisi per categorie.
...
19. Servizio Clienti: Assistenza telefonica ed email dedicata per prenotazione tagliandi e informazioni sul contratto.`
  ...
  {
    title: "Funzionamento del Contratto NLT (Durata, KM, Anticipo, Penali e Recesso)",
    source: "manual",
    content: `Il contratto di Noleggio a Lungo Termine si basa su parametri definiti al momento della firma che determinano il canone mensile: la durata del noleggio, il chilometraggio totale consentito, e la presenza di un eventuale anticipo finanziario.
...
Il recesso anticipato dal contratto è sempre possibile, ma comporta l'addebito di una penale calcolata in base ai mesi rimanenti alla scadenza. La penale standard applicata dalla maggior parte dei carrier (come Arval o Ayvens) è pari al 30-35% dei canoni mensili residui che il cliente avrebbe dovuto pagare fino alla fine naturale del contratto. Esistono formule particolari come "Be Free" di Leasys che azzerano questa penale dopo il 12° o 24° mese di noleggio, ottimali per chi non è sicuro delle proprie esigenze future.`
  ...
  {
    title: "Vantaggi Fiscali del Noleggio a Lungo Termine per P.IVA e Aziende",
    source: "manual",
    content: `Il Noleggio a Lungo Termine offre importanti vantaggi fiscali ed economici per le aziende, i liberi professionisti e i lavoratori autonomi con Partita IVA in Italia, consentendo di dedurre i costi e detrarre l'IVA in base all'effettivo utilizzo del veicolo.
...
documents.forEach((doc, docIdx) => {
  const docId = getDocUuid(docIdx + 1);
  
  // Scappa i singoli apici per SQL
  const safeTitle = doc.title.replace(/'/g, "''");
  const safeContent = doc.content.replace(/'/g, "''");
  
  sql += `-- ── Documento ${docIdx + 1}: ${doc.title} ──\n`;
  sql += `INSERT INTO knowledge_documents (id, title, content, source, created_by, is_active) \n`;
  sql += `VALUES (\n`;
  sql += `  '${docId}'::uuid,\n`;
  sql += `  '${safeTitle}',\n`;
  sql += `  '${safeContent}',\n`;
  ...
  sql += `  NULL,\n`;
  sql += `  true\n`;
  sql += `) ON CONFLICT (id) DO UPDATE SET title = EXCLUDED.title, content = EXCLUDED.content;\n\n`;

  // Suddividi in chunks
  const chunks = chunkText(doc.content);
  chunks.forEach((chunk, chunkIdx) => {
    const chunkId = getChunkUuid(docIdx + 1, chunkIdx + 1);
    const safeChunk = chunk.replace(/'/g, "''");
    const metadata = JSON.stringify({ index: chunkIdx, total: chunks.length });
    
    sql += `INSERT INTO knowledge_chunks (id, document_id, content, metadata) \n`;
    sql += `VALUES (\n`;
    sql += `  '${chunkId}'::uuid,\n`;
    sql += `  '${docId}'::uuid,\n`;
    sql += `  '${safeChunk}',\n`;
    sql += `  '${metadata}'::jsonb\n`;
    sql += `) ON CONFLICT (id) DO UPDATE SET content = EXCLUDED.content, metadata = EXCLUDED.metadata;\n\n`;
  });
  
  sql += `\n`;
});

// Aggiunge log finale per Supabase SQL Editor
sql += `SELECT count(*) as total_documents FROM knowledge_documents;\n`;
sql += `SELECT count(*) as total_chunks FROM knowledge_chunks;\n`;

const outPath = path.join(__dirname, '../supabase/migrations/20260622_seed_knowledge_base.sql');
fs.writeFileSync(outPath, sql, 'utf8');
console.log(`✅ Seed migration file generated successfully with valid UUIDs at: ${outPath}`);
